...
sdbcs_results = read.table("../results/all_x5_sdbcs/real_avg_execution.csv",header = TRUE)
...
ggplot(results, aes(x=id, y=time, group = type, colour=as.factor(algorithm))) + geom_point() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x = "Task ID", y = "Time in ms", colour = "Algorithms") + ylim(0, 5000)
